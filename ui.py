...
class UI:
    def __init__(self, monster, player_monsters, simple_surfs, get_input):
        ...
        #icones 
        self.elementos_icones = folder_importer('images', 'icones')

    def input(self):
        keys = pygame.key.get_just_pressed()
        if self.state == 'general':
            self.general_index['row'] = (self.general_index['row'] + int(keys[pygame.K_DOWN]) + int(keys[pygame.K_s]) - int(keys[pygame.K_UP]) - int(keys[pygame.K_w])) % self.rows
            self.general_index['col'] = (self.general_index['col'] + int(keys[pygame.K_RIGHT]) + int(keys[pygame.K_d]) - int(keys[pygame.K_LEFT]) - int(keys[pygame.K_a])) % self.cols
            if keys[pygame.K_SPACE]:
                self.state = self.general_options[self.general_index['col'] + self.general_index['row'] * 2]
        
        elif self.state == 'attack':
            self.attack_index['row'] = (self.attack_index['row'] + int(keys[pygame.K_DOWN]) + int(keys[pygame.K_s]) - int(keys[pygame.K_UP]) - int(keys[pygame.K_w])) % self.rows
            self.attack_index['col'] = (self.attack_index['col'] + int(keys[pygame.K_RIGHT]) + int(keys[pygame.K_d]) - int(keys[pygame.K_LEFT]) - int(keys[pygame.K_a])) % self.cols
            if keys[pygame.K_SPACE]:
            ...
            if self.available_monsters:
                self.switch_index = (self.switch_index + int(keys[pygame.K_DOWN] + int(keys[pygame.K_s]) - int(keys[pygame.K_UP] + int(keys[pygame.K_w])))) % len(self.available_monsters)
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                ...
                if self.state == "attack":
                    ability_name = options[i]
                    elemento = ABILITIES_DATA[ability_name]['element']
                    icon = self.elementos_icones[elemento]
                    icon_rect = icon.get_rect(midright=(text_rect.left + 110, y - 35))
