...
class Monster(pygame.sprite.Sprite, Creature):
    def __init__(self, name, surf):
        super().__init__()
        self.image = surf
        self.rect = self.image.get_rect(bottomleft = (100, WINDOW_HEIGHT)) # Position at the bottom left
        self.name = name
        self.get_data(name)
...
class Opponent(pygame.sprite.Sprite, Creature):
    def __init__(self, name, surf, groups):
        ...
        self.image = surf
        self.rect = self.image.get_rect(midbottom = (WINDOW_WIDTH - 250, 300)) # Position at the bottom right;
        self.get_data(name)